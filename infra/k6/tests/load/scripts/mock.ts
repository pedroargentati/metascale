...
const BASE_URL = 'http://localhost:8081'; // Atualize com a URL base da sua API
const ENDPOINT = '/api/metascale/users/{user_id}/products'; // Endpoint específico
...
export default function () {
	...
	for (let i = 50; i < 60; i++) {
		const res = http.get(`${BASE_URL}${ENDPOINT.replace('{user_id}', clientId.toString())}`);
