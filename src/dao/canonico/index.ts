import Canonico, { ICanonico } from '../../models/canonico';

export const getCanonicoBanco = async (): Promise<ICanonico[]> => {
	try {
		const canonicos: ICanonico[] = await Canonico.find();
		return canonicos;
	} catch (error: any) {
		console.error('Erro ao buscar os canônicos:', error.message);
		return [];
	}
